...
"""
ADS API Router - Unified Version

Endpoints para gestión de campañas de ads con optimización automática.
Incluye gestión de integraciones, campañas y creatives.
"""
...
class CreateAdCreativeRequest(BaseModel):
    """Add creative to existing campaign"""
    headline: str = Field(..., min_length=1, max_length=255)
    description: str = Field(..., max_length=1000)
    image_url: Optional[str] = None
    video_url: Optional[str] = None
...
async def create_campaign(
    ...
    Creatives are created along with the campaign.
    """
...
async def delete_campaign(
    ...
    Sets campaign status to 'archived' instead of deleting from database.
    """
...
@router.post("/campaigns/{campaign_id}/creatives", response_model=CreativeResponse, status_code=201)
async def create_ad_creative(
    ...
    Creates a new ad creative and initializes its Thompson Sampling state.
    The algorithm will automatically learn which creatives perform best.
    """
    ...
    # Create creative with Thompson Sampling state
    from engine.state.encryption import get_encryptor
    encryptor = get_encryptor()
    
    # Initial Thompson Sampling state (Beta distribution parameters)
    initial_state = {
        'success_count': 1,  # Prior: optimistic initialization
        'failure_count': 1,
        'samples': 0,
        'algorithm_type': 'bayesian'
    }
...
async def list_campaign_creatives(
    ...
    Returns creative details with current performance metrics.
    """
    ...
    async with db.pool.acquire() as conn:
        creatives = await conn.fetch(
            """
            SELECT 
                id, campaign_id, headline, description,
                image_url, video_url, cta_text, status,
                impressions, clicks, conversions,
                ctr, cpc, spend, created_at, updated_at
            FROM ad_creatives
            WHERE campaign_id = $1
...
@router.patch("/campaigns/{campaign_id}/creatives/{creative_id}/status")
async def update_creative_status(
    ...
    async with db.pool.acquire() as conn:
        result = await conn.execute(
            ...
            """,
            new_status, creative_id, campaign_id
        )
